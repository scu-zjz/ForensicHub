...
from UniForensicsBench.common.evaluation import AbstractEvaluator
from UniForensicsBench.common.utils.dataset import denormalize
...
def test_one_loader(model: torch.nn.Module,
    ...
    for data_iter_step, data_dict in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if BATCHSIZE != args.test_batch_size:
            print("=" * 20)
            print(
                f"A batch that is not fully loaded was detected at the end of the dataset. The actual batch size for this batch is {BATCHSIZE}: The default batch size is {args.test_batch_size}")
            print("=" * 20)
        ...
        for evaluator in evaluator_list:
            ...
            if results == None:  # Image-level results, do nothing
                ...
                assert BATCHSIZE == len(
                    results), f"Length of output results in evaluator {evaluator.name} does not match with the bachsize."
