...
class ForgeryAwareAdapterLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if up.shape[0] != dwt_feature.shape[0]:
            dwt_feature = dwt_feature.permute(1, 2, 0)
            dwt_feature = F.interpolate(dwt_feature, size = up.shape[0], mode='linear', align_corners=True)
            dwt_feature = dwt_feature.permute(2,0,1)
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, add_adapter=False,args=None):
        ...
        if add_adapter:
            self.forgery_aware_adapter = ForgeryAwareAdapterLayer(d_model=args.d_model)
        else:
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, add_adapter=None, args=None):
        ...
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask, add_adapter[idx],args) for idx in range(layers)])
