...
from ForensicHub.tasks.document.datasets.Others import OthersData
def get_args_parser():
...
def main(args, model_args, train_dataset_args, test_dataset_args, transform_args, evaluator_args):
    ...
    print('Direct RTM')
    use_dct=True
    train_dataset = OthersData('/mnt/data0/public_datasets/Doc/RealTextManipulation/RTM_train', train=True, use_dct=use_dct, edge_width=7)
    test_dataset_list = {}
    test_dataset_list['RTM_Test'] = OthersData('/mnt/data0/public_datasets/Doc/cutted_datasets_fakes/RealTextManipulation_test', train=False, use_dct=use_dct,crop_test=False,edge_width=7)
    test_sampler = {}
    if args.distributed:
