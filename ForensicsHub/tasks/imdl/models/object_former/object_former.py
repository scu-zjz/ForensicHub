...
from ForensicsHub.core.base_model import BaseModel
from ForensicsHub.registry import register_model
...
class BCIM(nn.Module):
    ...
    def forward(self, patch_embeddings):
        ...
        height = width = int(
            (sequence_length / 2) ** 0.5)  # Assuming square shape and divided by 2 due to concatenation

        # Reshape patch embeddings to 2D feature map
        feature_map = patch_embeddings.permute(0, 2, 1).reshape(batch_size, 2 * channels, height,
                                                                width)  # Shape: [batch_size, 2*embedding_dim, height, width]
        ...
        unfolded = unfolded.reshape(batch_size, 2 * channels, self.window_size ** 2, height,
                                    width)  # Shape: [batch_size, 2*embedding_dim, window_size*window_size, height, width]
        ...
        cosine_similarity = F.cosine_similarity(center_pixel, unfolded,
                                                dim=1)  # Shape: [batch_size, window_size*window_size, height, width]

        # Average similarity within the window
        similarity_matrix = cosine_similarity.mean(dim=1, keepdim=True) / (
                    self.window_size ** 2)  # Shape: [batch_size, 1, height, width]
        ...
        patch_embeddings = boundary_sensitive_feature_map.reshape(batch_size, -1, 2 * height * width).permute(0, 2,
                                                                                                              1)  # Shape: [batch_size, 2* height*width,  embedding_dim]
...
@register_model("ObjectFormer")
class ObjectFormer(nn.Module):
    def __init__(self, patch_size: int = 16, input_size: int = 224, num_prototypes: int = 392, embedding_dim: int = 768,
                 num_layers: int = 8, init_weight_path: str = None):
        ...
        self.encoder_decoders = nn.ModuleList(
            [EncoderDecoder(num_prototypes, embedding_dim) for _ in range(num_layers)])
    ...
    def _load_pretrain(self, init_weight_path):
        ...
        for i, block in enumerate(self.encoder_decoders):
            block.encoder.load_state_dict(
                {k.replace(f'blocks.{i}.', ''): v for k, v in state_dict.items() if f'blocks.{i}.' in k}, strict=False)
            block.decoder.load_state_dict(
                {k.replace(f'blocks.{i}.', ''): v for k, v in state_dict.items() if f'blocks.{i}.' in k}, strict=True)
        self.encoder_net_r[0].load_state_dict(
            {k.replace('patch_embed.proj.', ''): v for k, v in state_dict.items() if 'patch_embed' in k})
        self.encoder_net_q[0].load_state_dict(
            {k.replace('patch_embed.proj.', ''): v for k, v in state_dict.items() if 'patch_embed' in k})
    ...
    def forward(self, image, mask, label, *args, **kwargs):
        ...
        refined_patches = patch_embeddings.permute(0, 2, 1).reshape(batch_size, -1, height,
                                                                    width)  # Shape back to [batch_size, embedding_dim, height, width]
