...
class ResNet(BaseModel):
    ...
    def load_state_dict(self, pretrained_dict):
        ...
        filtered_dict = {k: v for k, v in pretrained_dict.items() if not k.startswith("fc.") and k in model_dict}
...
class Resnet101(ResNet):
    def __init__(self, pretrained=False, **kwargs):
        super().__init__(block=Bottleneck, layers=[3, 4, 23, 3], **kwargs)
        if pretrained:
...
class Resnet152(ResNet):
    def __init__(self, pretrained=False, **kwargs):
        super().__init__(block=Bottleneck, layers=[3, 8, 36, 3], **kwargs)
        if pretrained:
